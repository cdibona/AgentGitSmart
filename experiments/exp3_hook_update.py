...
def _install_hook(bare_repo: str) -> str:
    ...
    ctags = subprocess.run(["which", "ctags"], capture_output=True, text=True).stdout.strip() or "ctags"
    with open(hook_path, "w") as fh:
        fh.write(HOOK_TEMPLATE.format(root=str(_ROOT), python=sys.executable, ctags=ctags))
    os.chmod(hook_path, 0o755)
    return hook_path


def _human_push(bare_repo: str, work: str, branch: str, fname: str, content: str, msg: str) -> dict:
...
def run(branch: str = "main") -> dict:
    ...
    try:
        ...
        assert uninstall_mod.find_cache_refs(repo, REF_PREFIX) == [], "should start empty"

        # 2. + 3. first human push → hook should build a cache for the new commit
        p1 = _human_push(
            bare, work, branch, "hello.py",
            "# hello\nclass Greeter:\n    pass\n", "add Greeter",
        )
        repo = pygit2.Repository(bare)
        ...
        if built1:
            raw = cache_writer.read_artifact(repo, p1["new_oid"], "manifest.json", ref_prefix=REF_PREFIX)
            paths = {e["path"] for e in json.loads(raw)["entries"]}
            path_in_manifest1 = "hello.py" in paths
        steps.append({
            "step": "first_push", "new_oid": p1["new_oid"],
            "cache_built": built1, "edited_path_in_manifest": path_in_manifest1,
            "hook_stderr": p1["hook_stderr"],
        })
        print(f"[1] human push {p1['new_oid'][:12]} -> cache_built={built1} "
              f"path_in_manifest={path_in_manifest1}")
        print(f"    hook said: {p1['hook_stderr']}")

        # 4. second human push → a second, distinct cache ref appears
        p2 = _human_push(
            bare, work, branch, "world.py",
            "# world\ndef compute():\n    return 42\n", "add compute",
        )
        repo = pygit2.Repository(bare)
        ...
        steps.append({
            "step": "second_push", "new_oid": p2["new_oid"],
            "cache_built": built2, "total_cache_refs": total_refs,
            "hook_stderr": p2["hook_stderr"],
        })
        print(f"[2] human push {p2['new_oid'][:12]} -> cache_built={built2} "
              f"total_cache_refs={total_refs}")
        print(f"    hook said: {p2['hook_stderr']}")

        verdict = (
            "PASS" if (built1 and path_in_manifest1 and built2 and total_refs == 2)
            else "FAIL"
        )
